...
Character.prototype.createBranch = function(points, challenge){
    ...
    if (this.attribute - challenge <= 0) console.log('-1 Dice');
        ...
        probArr.push([(20-this.attribute-usablePoints+challenge)/20,1,0]) // nie wychodź poza skalę
        console.log(probArr)
        return probArr
    }
}
...
Character.prototype.oneRoll = function (challenge = 0, roll = 1, fails = 0, points = this.skill) {
    let probArr = this.createBranch(points, challenge) //try catch
    let sum = 0;
...
const character = new Character(17,0)
console.log(character.oneRoll(0))
